document.addEventListener("DOMContentLoaded", function () {
  ...
  if (hamburger && navLinks) {
    const toggleMenu = (force = null) => {
      const isActive =
        force !== null ? force : !hamburger.classList.contains("active");
    ...
    // --- Navigation menu links (Fixed version) ---
    navLinks.querySelectorAll("a").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        // Normal navigation for internal pages (like index.html or vintage.html)
  ...
  if (gallery && prevBtn && nextBtn) {
    ...
    if (firstCard) {
      const scrollGallery = (direction) => {
        ...
        const scrollAmount =
          direction === "next" ? cardWidth + cardGap : -(cardWidth + cardGap);
        gallery.scrollBy({
          left: scrollAmount,
          behavior: "smooth",
        });
      ...
      const updateArrowVisibility = () => {
        ...
        nextBtn.style.opacity =
          gallery.scrollLeft >= scrollEnd - 1 ? "0.5" : "1";
        nextBtn.style.cursor =
          gallery.scrollLeft >= scrollEnd - 1 ? "default" : "pointer";
      };
      ...
      fetch("/api/send-email", {
        ...
        .then((result) => {
          if (result.success) {
            ...
            alert(
              result.message || "Error sending message. Please try again later."
            );
          }
        })
        .catch((error) => {
